...
import 'package:movietime/bloc/get_airing_tv_show_bloc.dart';
...
class _AiringTodayState extends State<AiringToday> {
  ...
  void initState() {
    super.initState();
    nowAiringShowBloc..getAiringTVShow();
  }

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<AiringTVResponse>(
      stream: nowAiringShowBloc.subject.stream,
      builder: (context, AsyncSnapshot<AiringTVResponse> snapshot) {
        if (snapshot.hasData) {
  ...
  Widget _buildHomeWidget(AiringTVResponse data) {
    List<AiringTV> tv = data.tv;
    if (tv.length == 0) {
      return Container(
